...
public class EntryPoint {
    ...
    void DoBatchRender(ArrayList selectedTemplates, String basePath, RenderMode renderMode)
    {
        ...
        foreach (RenderItem renderItem in selectedTemplates)
        {
            ...
            if (RenderMode.Regions == renderMode) {
                int regionIndex = 0;
                foreach (ScriptPortal.Vegas.Region region in SelectedRegions) {
    ...
    DialogResult ShowBatchRenderDialog()
    {
        ...
        dlog.Text = "Batch Render Regions";
		...
		//Add region tree box
		RegionTree = new TreeView();
        RegionTree.Left = 10;
        RegionTree.Width = dlog.Width - 35;
        RegionTree.Top = TemplateTree.Bottom + 10;
        RegionTree.Height = (int)(300 * dpiScale);
        RegionTree.CheckBoxes = true;
        RegionTree.AfterCheck += new TreeViewEventHandler(this.HandleTreeViewCheck);
        dlog.Controls.Add(RegionTree);

        int buttonTop = RegionTree.Bottom + 16;
